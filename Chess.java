...
public class Chess extends JFrame implements Observer {
	...
	/**
	 * Generates main frame, panels, buttons, and sets their respective values.
	 * 
	 * @param model
	 *            Back end for GUI
	 */
	public Chess(ChessModel model) {
		this.model = model;
		model.addObserver(this); // allows model to push notify GUI of updates

		this.moveDisplay = new JTextArea("Move: " + model.getMoves()
		...
		center.setLayout(new GridLayout(model.getBoardY(), model.getBoardX(),
				5, 5));
	...
	private class ButtonListener implements ActionListener {
		...
		/**
		 * Takes in a reference to a button.
		 * 
		 * @param b
		 *            Button to listen on.
		 */
		...
		public void actionPerformed(ActionEvent e) {
			if (b instanceof ChessButton) { // if its a card, flip it over
			...
			} else { // its the tutorial button
				JOptionPane.showMessageDialog(gui,
					    "All the pieces move like normal chess pieces if \n" +
